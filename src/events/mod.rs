...
//!
//! - [`AttemptFailed`](EventKind::AttemptFailed) and [`AttemptTimedOut`](EventKind::AttemptTimedOut) describe one attempt. A later attempt may still run.
//! - [`TaskFinished`](EventKind::TaskFinished) carries the final outcome class. Registry cleanup then attempts [`TaskRemoved`](EventKind::TaskRemoved).
//! - Cancellation between attempts can reach `TaskFinished(Canceled)` without an [`AttemptCanceled`](EventKind::AttemptCanceled) event.
//! - [`Event::seq`] records process-local construction order. It is not a causal clock, and gaps are expected when events are dropped.
//! - [`Event::reason`] is diagnostic text. Use typed enums and their stable `as_label` methods for machine decisions and telemetry.
//! - Rejected work never enters the registry. It has no `TaskFinished` or `TaskRemoved` event.
