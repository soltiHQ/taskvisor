...
//!
//! [`SupervisorHandle::prepare_submission`](crate::SupervisorHandle::prepare_submission) allocates
//! a [`TaskId`] and returns it with the unsubmitted [`ControllerSpec`]. This lets an application
//! map its own ID to the `TaskId` before any event for that task can be published.
//!
//! ```text
//! prepare_submission
...
//! ```
//!
//! Preparation does not reserve the task name, slot, queue capacity, or runtime capacity.
...
///
/// Each submit method consumes this value, including when intake returns an error.
/// Retrying requires a new prepared value and a new task ID.
/// Dropping a prepared value without submitting starts no work and publishes no event.
///
/// # Examples
///
/// ```rust,no_run
...
impl PreparedSubmission {
    ...
    ///
    /// No event for this ID is published before a submit method consumes this value.
    /// After intake, the same ID identifies controller admission, events, cancellation, and the final outcome.
    /// An admitted runtime task uses it too.
    ///
    ...
    ///
    /// The returned ID is the value from [`id`](Self::id). Success confirms command intake.
    /// Slot admission and runtime registration happen later. Use [`submit_and_watch`](Self::submit_and_watch)
    /// when the caller needs the later admission rejection or final task outcome.
    ///
    /// # Errors
    ///
    /// Returns [`ControllerError::ThreadStartFailed`] or [`ControllerError::ResourceLimit`] when Taskvisor cannot reserve
    /// cleanup ownership for the task. Returns [`ControllerError::Closed`] when the controller command channel closes before intake.
    ...
    /// # Errors
    ///
    /// Returns the intake errors from [`submit`](Self::submit).
    /// It also returns [`ControllerError::Full`] when the command queue has no capacity.
    ...
    ///
    /// Success confirms command intake. The waiter later reports [`TaskOutcome::Rejected`](crate::TaskOutcome::Rejected)
    /// for admission rejection, or the final outcome of an admitted task. Call [`TaskWaiter::wait`] to receive it.
    ///
    /// # Errors
    ///
    ...
    ///
    /// Success confirms command intake. The returned waiter has the same contract as [`submit_and_watch`](Self::submit_and_watch).
    /// Use this method when the caller needs the final result and owns its backpressure policy.
    ///
    /// # Errors
    ///
    /// Returns the intake errors from [`submit_and_watch`](Self::submit_and_watch).
    /// It also returns [`ControllerError::Full`] when the command queue has no capacity.
