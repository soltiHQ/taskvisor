...
//! and retains labels for grace-expiry diagnostics.
//!
...
/// Mutable counts and labels for removal owners awaiting terminal cleanup.
#[derive(Default)]
struct PendingInner {
    /// Number of removal owners still active for each task.
    counts: HashMap<TaskId, usize>,
    /// Task labels retained for shutdown diagnostics.
...
pub(in crate::core::registry) struct PendingJoins {
    /// Counts and labels protected from concurrent removal owners.
...
impl PendingJoins {
    /// Registers one removal owner and its diagnostic label atomically.
    ...
    /// Registers one removal owner without a label for a focused test.
    #[cfg(test)]
    ...
    /// Attaches a diagnostic label to an existing test owner.
    #[cfg(test)]
    ...
    /// Returns labels for removal owners still active at a shutdown deadline.
