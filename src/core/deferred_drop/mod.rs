...
//!
//! `SupervisorBuilder` creates one [`DropDomain`] for each supervisor. Runtime task admission,
//! controller submission, and subscriber construction reserve this domain before Taskvisor
//! accepts their user-owned values. Controller, registry, subscriber, and reaper paths
//! carry its bundle until their final retained values are ready for destruction.
//!
//! ```text
...
//! ```
//!
//! This isolation prevents a blocking or panicking destructor from running on a Tokio worker,
//! the registry listener, or a controller loop. Cleanup runs on dedicated operating-system threads
//! after internal ownership ends. User destructors do not run under the bundle, capacity, or worker-queue locks.
