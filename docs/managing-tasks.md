...
| `submit`           | The controller accepted the command. The return is not an admission decision. |
...
| `remove`           | `true` means this call started removal. Registered cleanup may continue; queued work is removed before return. |
| `cancel`           | `true` means this call started removal. Registered work has left the registry and its final outcome is settled before return. |
...
After a force-abort, physical task code may still be running even after `cancel` returns.
