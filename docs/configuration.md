...
At registration, Taskvisor fills inherited settings from `TaskDefaults`.
The `TaskSpec` constructor decides which fields inherit:
...
| `once`, `restartable`, `periodic` | Chosen by the constructor. | Backoff, timeout, and retry limit are inherited. |
...
- [with_timeout(None)](https://docs.rs/taskvisor/latest/taskvisor/tasks/struct.TaskSpec.html#method.with_timeout) disables the attempt timeout. `Duration::ZERO` has the same effect.
...
To disable retries, use `TaskSpec::once` or `RestartPolicy::Never`.
...
Use `Supervisor::ownership_snapshot` or [SupervisorHandle::ownership_snapshot](https://docs.rs/taskvisor/latest/taskvisor/core/struct.SupervisorHandle.html#method.ownership_snapshot)
...
`waiters` counts ownership requests waiting for capacity.
`cleanup_queued` and `cleanup_running` count batches waiting for or running on destructor workers.
`retired()` reports permanent loss from a finite ownership capacity after destructor failure.
An `available` count of zero does not by itself mean the next waiting request will fail. Another owned value may release its unit.
